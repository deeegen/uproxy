...
module.exports = class {
  constructor(prefix = "/web/", config = {}) {
    ...
    // Modified: Accept userId as third argument for per-user URL encoding
    this.proxifyRequestURL = (url, type, userId = "") =>
      type
        ? atob(url.split("_").slice(1).splice(0, 1).join())
            .split("|")
            .slice(1)
            .join("|") + url.split("_").slice(2).join("_")
        : `_${btoa(userId + "|" + url.split("/").splice(0, 3).join("/"))}_/${url
            .split("/")
            .splice(3)
            .join("/")}`;
  ...
  http(req, res, next = () => res.end("")) {
    ...
    // Modified: Get userId from header or fallback to empty string
    const userId = req.headers["x-user-id"] || "";

    try {
      new URL(this.proxifyRequestURL(req.path, true, userId));
    } catch {
      return res.end("URL Parse Error");
    }

    var proxyURL = {
        href: this.proxifyRequestURL(req.path, true, userId),
        origin: this.proxifyRequestURL(req.path, true, userId)
          .split("/")
          .splice(0, 3)
          .join("/"),
        hostname: this.proxifyRequestURL(req.path, true, userId)
          .split("/")
          .splice(0, 3)
          .slice(2)
          .join("/"),
    ...
    if (!req.path.startsWith(`_${btoa(userId + "|" + proxyURL.origin)}_`))
      return (
        res.writeHead(308, {
          location: this.prefix + `_${btoa(userId + "|" + proxyURL.origin)}_/`,
        }),
        res.end("")
      );
          ...
          .on("end", () => {
            ...
            proxify.url = (url) => {
              ...
              if (url.protocol == "https:" || url.protocol == "http:")
                return (
                  this.prefix + this.proxifyRequestURL(url.href, false, userId)
                );
